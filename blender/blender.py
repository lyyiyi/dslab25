...
def make_silver_material():
    ...
    Adjust Base Color, Metallic, and Roughness for a less shiny look.
    """
    ...
    # Increase roughness to reduce shine (from 0.2 to 0.6)
...
def setup_camera():
    """Create a camera at (0,-10,10) and set lens to 35mm."""
...
def render_views(obj, cam, output_dir, drop_index):
    """Render 4 views from a circle of radius 9 at height 7, pointing at (0,0,0)."""
    scene = bpy.context.scene
    num_views = 4
    for i in range(num_views):
        angle = 2 * math.pi * i / num_views
        radius = 9
        cam.location = (radius * math.cos(angle), radius * math.sin(angle), 7)
