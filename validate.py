...
checkpoints = {}
# yolo checkpoints
checkpoints["yolo"] = ["/Users/owen/Projects/dslab25/obj_detection/yolo/yolov9_finetune2/weights/best.pt"]
models = {}
models["yolo"] = YOLO

# paths
video_paths = [f"/Users/owen/Projects/dslab25/training/vacuum_pump/videos/01_run{run}_cam_{cam}_1024x1024_15fps_3mbps.mp4" \
    for run in (1,2,3) for cam in (2,3)]
checkpoint_path = "/Users/owen/Projects/dslab25/obj_detection/yolo/yolov9_finetune2/weights/best.pt"
st.session_state.model = YOLO(checkpoint_path)
st.session_state.cap = None # video capture
st.session_state.cap = cv2.VideoCapture(video_paths[0])
st.session_state.total_frames = int(st.session_state.cap.get(cv2.CAP_PROP_FRAME_COUNT))
